...
function cardHTMLCliente(p) {
  ...
  return `
    <article class="card" data-id="${p.id}" data-href="/produto/${p.id}/" data-nome="${escapeHTMLJS(normalizarBuscaJS(p.nome))}" data-categoria="${escapeHTMLJS(p.categoria || "")}" data-material="${escapeHTMLJS(material)}" data-pix="${temPix ? "1" : "0"}" data-preco="${p.preco}" data-cliques="${p.cliques_total || 0}" data-criado="${p.criado_em || ""}">
      ...
      <div class="card-corpo">
        <h3 class="card-nome" title="${escapeHTMLJS(p.nome)}">${escapeHTMLJS(p.nome)}</h3>
